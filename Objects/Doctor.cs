...
namespace Hospital.Objects
{
  public class Doctor
  {
    ...
    public static List<Doctor> CombineSearch(string nameSearch, string specialtySearch)
    {
      DB.CreateConnection();
      DB.OpenConnection();

      SqlCommand cmd = new SqlCommand("SELECT * FROM doctors WHERE name LIKE @NameSearch AND specialty LIKE @SpecialtySearch", DB.GetConnection());
      cmd.Parameters.Add(new SqlParameter("@NameSearch", "%" + nameSearch + "%"));
      cmd.Parameters.Add(new SqlParameter("@SpecialtySearch", "%" + specialtySearch + "%"));
